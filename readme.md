...
| `git diff --staged` ou `git diff --cached` | **Voir les changements entre HEAD ce qui a été stage** |
| `git diff <fichier1> <fichier2> <etc...>` | **Voir les changements dans le où les fichiers ciblés.** |
|  `git diff <branche1> <branche2>` ou `git diff <branche1>..<branche2>` | **Voir les changements dans un intervalle de branches (par rapport au dernier commit qu'elles pointent).** <br> possibilité de faire la comparaison seulement sur certains fichiers ren rajoutant leurs noms à la suite.
| `git diff <commit1> <commit2>` ou `git diff <commit1>..<commit2>`|**Voir les changements entre 2 commits.** <br> Il est aussi possible de ne citer qu'un seul commit ça fera une comparaison entre HEAD et celui-ci. Par exemple `git diff HEAD~1` va comparer les changements entre le dernier commit et HEAD.
